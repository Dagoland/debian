...
su -c "echo 'deb http://ftp.debian.org/debian/ testing main contrib non-free' > /etc/apt/sources.list.d/testing.list"
su -c "echo 'deb-src http://ftp.debian.org/debian/ testing main contrib non-free' >> /etc/apt/sources.list.d/testing.list"
...
apt-get -t testing install mate-desktop -y --force-yes
